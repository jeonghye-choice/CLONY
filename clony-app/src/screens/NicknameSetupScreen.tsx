...
const NicknameSetupScreen: React.FC<NicknameSetupScreenProps> = ({ onComplete }) => {
        ...
        <View className="flex-1 bg-white px-8 justify-center">
            ...
            <View className="mb-8">
                ...
                <View className="flex-row flex-wrap gap-2">
                    {['지민', '예진', 'Sarah', '수민이', '민지123'].map((example) => (
                        <TouchableOpacity
                            key={example}
            ...
            <TouchableOpacity
                onPress={handleNext}
                disabled={!isValid}
                className={`py-4 rounded-xl items-center shadow-lg ${isValid
                    ? 'bg-clony-primary shadow-green-200'
                    : 'bg-gray-200'
                    }`}
            >
...
export default NicknameSetupScreen;
