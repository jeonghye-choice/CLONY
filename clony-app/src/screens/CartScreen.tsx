...
import { View, Text, TouchableOpacity, ScrollView, Image, Alert, Modal, Platform } from 'react-native';
...
const CartScreen: React.FC<CartScreenProps> = ({ onCheckout, onBack }) => {
    ...
    const getDeliveryFee = () => {
        switch (deliveryOption) {
            case 'same-day': return 3000;
            case 'dawn': return 2500;
    ...
    const handleRemoveItem = (id: string) => {
        Alert.alert(
            t('cart.remove_confirm_title'),
            t('cart.remove_confirm_msg'),
            [
    ...
    const handleCheckoutPress = () => {
        if (cartItems.length === 0) {
            Alert.alert(t('cart.empty_title'), t('cart.empty_desc'));
            return;
        }
    ...
    const handleConfirmCheckout = () => {
        ...
        onCheckout(cartItems, finalTotal, deliveryOption, deliveryOption === 'hotel' ? hotelTime : undefined);
    };
        ...
        return (
            <View className="flex-1 bg-gray-50">
                ...
                <View className="flex-1 items-center justify-center px-8">
                    ...
                    >
                        <Text className="text-white font-bold">{t('cart.go_shopping')}</Text>
                    </TouchableOpacity>
                </View>
            </View>
        );
        ...
        <View className="flex-1 bg-gray-50">
                    ...
                    <TouchableOpacity
                        onPress={() => {
                            Alert.alert(
                                ...
                                t('cart.clear_confirm_msg'),
                                [
                                    { text: t('common.cancel'), style: 'cancel' },
                                    { text: t('cart.clear_all'), style: 'destructive', onPress: clearCart },
                                ]
                            );
                        }}
                    >
                        <Text className="text-sm text-gray-500">{t('cart.clear_all')}</Text>
                    </TouchableOpacity>
                </View>
            ...
            <ScrollView className="flex-1" contentContainerStyle={{ padding: 20, paddingBottom: 40 }}>
                                    ...
                                    {item.matchingScore && (
                                        <View className="bg-teal-50 self-start px-2 py-1 rounded-md mb-3 border border-teal-200">
                                            <Text className="text-teal-600 text-[10px] font-bold">
                                                {t('cart.match_percentage', { percentage: item.matchingScore })}
                                            </Text>
                                        </View>
                                    )}
                ...
                <View className="bg-white rounded-2xl p-5 mb-4 border border-gray-100">
                    <Text className="font-bold text-gray-900 mb-4 text-base">{t('cart.delivery_options')}</Text>

                    {/* Same-day Delivery */}
                    <TouchableOpacity
                        onPress={handleSameDaySelect}
                    ...
                    >
                        <Ionicons
                            ...
                            size={24}
                            color={deliveryOption === 'same-day' ? '#00A676' : '#9CA3AF'}
                        />
                        <View className="ml-3 flex-1">
                            <Text className="font-bold text-gray-900 text-base">🚚 {t('cart.same_day')}</Text>
                            <Text className="text-xs text-gray-500 mt-1">{t('cart.same_day_desc')}</Text>
                        </View>
                    ...
                    >
                        <Ionicons
                            ...
                            size={24}
                            color={deliveryOption === 'dawn' ? '#00A676' : '#9CA3AF'}
                        />
                        <View className="ml-3 flex-1">
                            <Text className="font-bold text-gray-900 text-base">🌙 {t('cart.dawn')}</Text>
                            <Text className="text-xs text-gray-500 mt-1">{t('cart.dawn_desc')}</Text>
                        </View>
                        <Text className="text-sm font-bold text-clony-primary">+2,500</Text>
                    </TouchableOpacity>
                    ...
                    >
                        <Ionicons
                            ...
                            size={24}
                            color={deliveryOption === 'hotel' ? '#00A676' : '#9CA3AF'}
                        />
                        <View className="ml-3 flex-1">
                            <Text className="font-bold text-gray-900 text-base">🏨 {t('cart.hotel')}</Text>
                            <Text className="text-xs text-gray-500 mt-1">
                                {deliveryOption === 'hotel'
                                    ? (parseInt(hotelTime.split(':')[0]) >= 19
                                        ? t('cart.hotel_desc_tomorrow')
                                        : t('cart.hotel_desc_today', { time: hotelTime }))
                                    : t('cart.hotel_desc_default')}
                            </Text>
                        </View>
                        {deliveryOption === 'hotel' && (
                            <TouchableOpacity
                            ...
                            >
                                <Text className="text-xs font-bold text-[#00A676]">{t('cart.set_time')}</Text>
                            </TouchableOpacity>
                        )}
                        {deliveryOption !== 'hotel' && (
                            <Text className="text-sm font-bold text-clony-primary">+3,000</Text>
            ...
            <View className="bg-white border-t border-gray-100 px-6 py-5 pb-8 rounded-t-3xl shadow-2xl">
                <View className="flex-row justify-between items-center mb-2">
                    <Text className="text-sm text-gray-600">{t('cart.price_summary.items')}</Text>
                    <Text className="text-base font-bold text-gray-900">
                        ₩{cartTotal.toLocaleString()}
                    </Text>
                </View>
                <View className="flex-row justify-between items-center mb-4 pb-4 border-b border-gray-100">
                    <Text className="text-sm text-gray-600">{t('cart.price_summary.delivery')}</Text>
                    <Text className="text-base font-bold text-clony-primary">+₩{deliveryFee.toLocaleString()}</Text>
                </View>
                <View className="flex-row justify-between items-center mb-4">
                    <Text className="text-base text-gray-900 font-bold">{t('cart.price_summary.total')}</Text>
                ...
                >
                    <Text className="text-white font-bold text-lg">
                        {t('cart.price_summary.checkout_btn', { amount: finalTotal.toLocaleString() })}
                    </Text>
                </TouchableOpacity>
            </View>
            ...
            >
                <View className="flex-1 bg-black/50 justify-center items-center px-6">
                    <View className="bg-white w-full rounded-2xl p-6 max-h-[80%]">
                        <Text className="text-xl font-bold text-gray-900 mb-4 text-center">{t('cart.time_picker.title')}</Text>
                        <Text className="text-xs text-gray-400 text-center mb-4">
                            {t('cart.time_picker.hint')}
                        </Text>
                                        ...
                                        <View className="flex-row justify-center items-center gap-2">
                                            ...
                                            {isSameDay && (
                                                <View className="bg-teal-100 px-2 py-0.5 rounded-md">
                                                    <Text className="text-[10px] text-teal-600 font-bold">{t('cart.time_picker.same_day_badge')}</Text>
                                                </View>
                                            )}
                                        </View>
                                    </TouchableOpacity>
                        ...
                        >
                            <Text className="font-bold text-gray-600">{t('common.close')}</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            </Modal>
                ...
                <View className="flex-1 bg-black/50 justify-center items-center px-6">
                    <View className="bg-white w-full rounded-2xl p-6 items-center shadow-2xl">
                        <View className="w-16 h-16 bg-teal-50 rounded-full items-center justify-center mb-4">
                            <Ionicons name="card" size={32} color="#00A676" />
                        </View>
                        <Text className="text-xl font-bold text-gray-900 mb-2">{t('cart.checkout_confirm.title')}</Text>
                        <Text className="text-gray-500 text-center mb-6">
                            {t('cart.checkout_confirm.desc', { count: cartItems.length, amount: finalTotal.toLocaleString() })}
                            {deliveryOption === 'hotel' && (
                                parseInt(hotelTime.split(':')[0]) >= 19
                                    ? t('cart.checkout_confirm.hotel_tomorrow')
                                    : t('cart.checkout_confirm.hotel_today', { time: hotelTime })
                            )}
                        </Text>
            ...
            {/* Delivery Warning Modal (After 6 PM) */}
            <Modal
                visible={showDeliveryWarning}
                transparent={true}
                animationType="fade"
                onRequestClose={() => setShowDeliveryWarning(false)}
            >
                <View className="flex-1 bg-black/50 justify-center items-center px-6">
                    <View className="bg-white w-full rounded-2xl p-6 items-center shadow-2xl">
                        ...
                        <Text className="text-xl font-bold text-gray-900 mb-2">{t('cart.same_day_unavailable_title')}</Text>
                        <Text className="text-gray-500 text-center mb-6 leading-5">
                            {t('cart.same_day_unavailable_msg')}
                        </Text>
                        <TouchableOpacity
                            onPress={() => {
                                setShowDeliveryWarning(false);
                                setDeliveryOption('dawn');
                            }}
                            className="w-full py-4 bg-clony-primary rounded-xl items-center shadow-md"
                        >
                            <Text className="text-white font-bold text-base">{t('common.confirm')}</Text>
                        </TouchableOpacity>
                    </View>
