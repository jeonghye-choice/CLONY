...
# EasyOCR Setup (Optional - 폴백용으로 유지)
try:
    import easyocr
...
# ─── OCR Utils (PaddleOCR + Fuzzy Matching) ───
try:
    from ocr_utils import run_best_ocr, PADDLEOCR_AVAILABLE, fuzzy_correct_ingredient
    print("[OCR-Utils] ocr_utils 모듈 로드 완료")
except Exception as _ocr_utils_err:
    PADDLEOCR_AVAILABLE = False
    print(f"[OCR-Utils] ocr_utils 로드 실패: {_ocr_utils_err}")
    def run_best_ocr(image):
        raise RuntimeError("ocr_utils 모듈을 사용할 수 없습니다.")
...
@app.post("/inquiry")
def submit_inquiry(req: InquiryRequest):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute(
        "INSERT INTO inquiries (user_email, category, title, content) VALUES (?, ?, ?, ?)",
        (req.user_email, req.category, req.title, req.content)
    )
    inquiry_id = c.lastrowid
    conn.commit()
    conn.close()
    return {"success": True, "id": inquiry_id, "status": "pending"}


class ReturnRequest(BaseModel):
    order_id: str
    order_items: str  # comma-separated item names
    request_type: str  # "return" or "exchange"
    reason: str
    user_email: str = ""

@app.post("/return-request")
def submit_return_request(req: ReturnRequest):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute('''
    ...
    ''')
    c.execute(
        "INSERT INTO return_requests (order_id, order_items, request_type, reason, user_email) VALUES (?, ?, ?, ?, ?)",
        (req.order_id, req.order_items, req.request_type, req.reason, req.user_email)
    )
    req_id = c.lastrowid
    conn.commit()
    conn.close()
    return {"success": True, "id": req_id, "status": "processing"}
...
def submit_review(req: ReviewRequest):
    ...
    ''')
    c.execute(
        "INSERT INTO reviews (product_name, brand, rating, content, user_email) VALUES (?, ?, ?, ?, ?)",
        (req.product_name, req.brand, req.rating, req.content, req.user_email)
    )
...
class PaymentRequest(BaseModel):
    ...
    order_name: str
    amount_krw: int
    payment_method: str  # credit_card, alipay, wechat_pay, paypal, apple_pay, google_pay
    currency: str = "KRW"
    user_email: str = ""
...
def process_payment(req: PaymentRequest):
    ...
    try:
        c.execute(
            "INSERT INTO payments (order_id, order_name, amount_krw, payment_method, currency, user_email) VALUES (?, ?, ?, ?, ?, ?)",
            (req.order_id, req.order_name, req.amount_krw, req.payment_method, req.currency, req.user_email)
        )
        conn.commit()
    except sqlite3.IntegrityError:
        # Duplicate order_id — already processed
        conn.close()
        return {"success": True, "order_id": req.order_id, "status": "already_processed"}
    conn.close()
    return {"success": True, "order_id": req.order_id, "status": "completed"}
...
INGREDIENT_EFFECTS = {
    ...
    # --- Emollient / Texture Agents ---
    "dibutyl adipate": {"effect": "피부 유연화/용제", "type": "모든 피부", "concern": "사용감", "nameKo": "다이부틸아디페이트", "component_type": "Emollient", "time_use": "Any"},
    "다이부틸아디페이트": {"effect": "피부 유연화/용제", "type": "모든 피부", "concern": "사용감", "nameKo": "다이부틸아디페이트", "component_type": "Emollient", "time_use": "Any"},
}
...
async def analyze_skin(file: UploadFile = File(...)):
    ...
    try:
        if GOOGLE_API_KEY == "YOU_MUST_ENTER_YOUR_API_KEY_HERE" or not GOOGLE_API_KEY:
        ...
        model_id = 'gemini-3.1-pro-preview' # Primary model requested by user
...
async def analyze_product(file: UploadFile = File(...)):
    ...
    # ============================================================
    # 2. OCR 파이프라인 (PaddleOCR → EasyOCR → Gemini Vision)
    # ============================================================
    ocr_text = None
    ocr_used = False
    extracted_ingredients = []

    # 2-1. run_best_ocr: PaddleOCR (우선) → EasyOCR (폴백)
    try:
        print("[OCR] run_best_ocr 실행 중 (PaddleOCR 우선)...")
        ocr_result = run_best_ocr(image)
        ocr_text = ocr_result["full_text"]
        extracted_ingredients = ocr_result["ingredients"]
        if ocr_text.strip():
            ocr_used = True
            print(f"[OCR] 성공 ({len(extracted_ingredients)}개 성분 검출), 미리보기: {ocr_text[:150]}")
        else:
            print("[OCR] 텍스트 미검출 → Gemini Vision 직접 분석으로 전환")
    except Exception as ocr_err:
        print(f"[OCR] run_best_ocr 실패: {ocr_err} → Gemini Vision으로 전환")
    ...
    try:
        ...
        # Try multiple models (Prioritize standard 2.5 flash in new genai SDK)
...
4. "ingredients": [ {{ "name": "English Name", "nameKo": "한글 성분명", "benefit": "English Benefit (e.g. Hydration)" }} ] 
   - [IMPORTANT] "name" 무조건 영어(English) 성분명만 적으세요.
   - [IMPORTANT] "nameKo" 무조건 한국어(Korean) 성분명만 적으세요.
   - 중요도에 상관없이 텍스트 내에서 검출된 모든 성분을 하나도 빠짐없이 리스트업하세요. (매우 중요)
   - benefit은 반드시 영어 단어나 아주 짧은 영어 구문으로 작성하세요 (예: Hydration, Soothing, Anti-aging).
5. "keyIngredients": [ {{ "name": "English Name", "nameKo": "한글 성분명", "benefit": "English Benefit" }} ]
   - [IMPORTANT] "name" 무조건 영어(English) 성분명만 적으세요.
   - [IMPORTANT] "nameKo" 무조건 한국어(Korean) 성분명만 적으세요.
...

{
    ...
    "category": "카테고리",
    "ingredients": [
        {"name": "Ingredient Name", "nameKo": "성분명", "benefit": "English Benefit (e.g. Hydration)"}
    ],
    "keyIngredients": [
        {"name": "Ingredient Name", "nameKo": "성분명", "benefit": "English Benefit (e.g. Soothing)"}
    ],
    "warnings": ["주의사항"],
    "summary": "AI 분석 요약",
...
5. [IMPORTANT] "name" 항목은 반드시 영어(English Input)로만 작성하고, "nameKo" 항목은 반드시 한국어(Korean)로만 작성하세요. "benefit" 항목은 반드시 영어(English)로만 작성하세요."""
                ...
                print(f"[Gemini] Cleaned JSON length: {len(text_response)}")
                ...
                try:
                    ...
                    print(f"[Gemini] JSON Decode Error ({model_name}): {j_err}")
                    print(f"[Gemini] Problematic JSON Text: {text_response}")
                    continue
...
# 성분 사전 검색 API
# ============================================================
@app.get("/ingredient-search")
def ingredient_search(query: str = "", page: int = 1, limit: int = 30):
    """
    ingredient_mapping.json + INGREDIENT_EFFECTS 기반 성분 검색.
    - query: 한글 또는 영문 성분명 (2글자 이상 권장)
    - 반환: [{nameKo, nameEn, effect, skinType, concern, componentType, timeUse}]
    """
    q = query.strip().lower()

    # 전체 성분 데이터 빌드 (한글 기준으로 합산)
    # ingredient_mapping: {ko: en}
    STANDARD_INGREDIENT_MAPPING_PATH = os.path.join(
        os.path.dirname(__file__),
        "../database/ingredients/ingredient_mapping.json"
    )
    try:
        with open(STANDARD_INGREDIENT_MAPPING_PATH, "r", encoding="utf-8") as f:
            ing_map: dict = json.load(f)
    except Exception:
        ing_map = {}
    ...
    # INGREDIENT_EFFECTS 우선 (효능 정보가 풍부)
    for key, info in INGREDIENT_EFFECTS.items():
        ko_name = info.get("nameKo", key)
        en_name = key if key.replace(" ", "").isascii() else info.get("nameEn", ing_map.get(ko_name, ko_name))

        if ko_name in seen_ko:
            continue

        # 검색어 매칭
        if q and q not in ko_name.lower() and q not in en_name.lower():
            continue

        seen_ko.add(ko_name)
        results.append({
            "nameKo": ko_name,
            "nameEn": en_name if en_name != ko_name else ing_map.get(ko_name, ko_name.upper()),
            "effect": info.get("effect", ""),
            "skinType": info.get("type", ""),
            "concern": info.get("concern", ""),
            "componentType": info.get("component_type", ""),
            "timeUse": info.get("time_use", ""),
            "conflicts": info.get("conflicts", []),
        })

    # ing_map에서 추가 (INGREDIENT_EFFECTS에 없는 것)
    for ko, en in ing_map.items():
        if ko in seen_ko:
            continue
        if q and q not in ko.lower() and q not in en.lower():
            continue
        seen_ko.add(ko)
        results.append({
            "nameKo": ko,
    ...
    # 정렬: 효능 있는 것 우선, 한글 이름 오름차순
    results.sort(key=lambda x: (0 if x["effect"] else 1, x["nameKo"]))

    total = len(results)
    start = (page - 1) * limit
    return {"total": total, "results": results[start:start + limit]}

# ============================================================
# Announcement and Notice Endpoints
# ============================================================
...
def get_recent_announcements():
    ...
    conn.row_factory = sqlite3.Row
    c = conn.cursor()
    c.execute("SELECT * FROM announcements ORDER BY is_important DESC, created_at DESC LIMIT 5")
    an = [dict(row) for row in c.fetchall()]
    conn.close()
    return an
...
def create_admin_announcement(an: AnnouncementCreate):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute('''
        INSERT INTO announcements (title, content, is_important)
        VALUES (?, ?, ?)
    ''', (an.title, an.content, an.is_important))
    conn.commit()
    conn.close()
...
def admin_stats():
    ...
    c.execute("SELECT COUNT(*) as cnt FROM inquiries WHERE status='pending'")
    pending_inquiries = c.fetchone()["cnt"]
    c.execute("SELECT COUNT(*) as cnt FROM payments")
    payments_count = c.fetchone()["cnt"]
    conn.close()
    return {
        "users": users_count,
        "inquiries": pending_inquiries,
        "orders": payments_count,
    }
...
def admin_payments():
    ...
    conn.row_factory = sqlite3.Row
    c = conn.cursor()
    c.execute("CREATE TABLE IF NOT EXISTS payments (id INTEGER PRIMARY KEY AUTOINCREMENT, order_id TEXT UNIQUE, order_name TEXT, amount_krw INTEGER, payment_method TEXT, currency TEXT DEFAULT 'KRW', status TEXT DEFAULT 'completed', user_email TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
...
@app.patch("/admin/inquiries/{inquiry_id}")
def admin_answer_inquiry(inquiry_id: int, req: AdminAnswerRequest):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("UPDATE inquiries SET answer=?, status='completed' WHERE id=?", (req.answer, inquiry_id))
    conn.commit()
    conn.close()
    return {"success": True}
...
def admin_returns():
    ...
    conn.row_factory = sqlite3.Row
    c = conn.cursor()
    c.execute("CREATE TABLE IF NOT EXISTS return_requests (id INTEGER PRIMARY KEY AUTOINCREMENT, order_id TEXT, order_items TEXT, request_type TEXT, reason TEXT, user_email TEXT, status TEXT DEFAULT 'processing', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)")
    c.execute("SELECT * FROM return_requests ORDER BY created_at DESC")
    rows = [dict(row) for row in c.fetchall()]
    conn.close()
    return rows

class AdminReturnStatusRequest(BaseModel):
    status: str

@app.patch("/admin/returns/{return_id}")
def admin_update_return(return_id: int, req: AdminReturnStatusRequest):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("UPDATE return_requests SET status=? WHERE id=?", (req.status, return_id))
    conn.commit()
    conn.close()
    return {"success": True}
